...
export default function Hall() {
  ...
  const handleJoin = (e) => {
    ...
    if (isHost && topic.trim()) params.set("topic", topic.trim());
      ...
      <div className="relative z-10 w-full max-w-lg p-6">
        ...
        >
          ...
          <form onSubmit={handleJoin} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                {['room-001', 'room-002', 'vip-lounge', 'dev-team'].map((r) => (
                  <div key={r} onClick={() => setRoom(r)} className={`cursor-pointer p-3 rounded-lg border text-xs font-mono text-center transition-all ${room === r ? "bg-blue-600/20 border-blue-500 text-blue-300" : "bg-white/5 border-white/5 hover:bg-white/10 text-gray-500"}`}>
                    #{r}
                  </div>
                ))}
              </div>
            ...
            <div className="pt-2">
              <div className={`flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-all ${isHost ? "bg-purple-900/20 border-purple-500/50" : "bg-white/5 border-white/5 hover:bg-white/10"}`} onClick={() => setIsHost(!isHost)}>
                <div>
              ...
              <AnimatePresence>
                {isHost && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden space-y-3 mt-3">
                    <div className="p-3 bg-red-900/10 border border-red-500/20 rounded-xl flex items-center gap-3">
                      <div className="text-red-500">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                      </div>
                      <input 
                        type="password" value={password} onChange={(e) => setPassword(e.target.value)}
                        className="bg-transparent border-none text-white text-sm w-full focus:outline-none placeholder-red-300/50" placeholder="Enter Host Password"
                      />
                    </div>
                    
                    <div className="p-3 bg-blue-900/10 border border-blue-500/20 rounded-xl flex items-center gap-3">
                      ...
                      <input 
                        ...
                        className="bg-transparent border-none text-white text-sm w-full focus:outline-none placeholder-blue-300/40" placeholder="Session Topic (AI Target)"
                      />
                    </div>
                  </motion.div>
                )}
            ...
            <button type="submit" disabled={!name} className={`w-full py-4 rounded-xl font-bold text-sm uppercase tracking-widest hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 shadow-xl ${isHost ? "bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-purple-900/20" : "bg-white text-black"}`}>
